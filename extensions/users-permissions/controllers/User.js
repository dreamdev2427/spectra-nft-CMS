...
module.exports = {
   ...
   * @return {Object|Array}
   */
  async find(ctx, next, { populate=null } = {}) {
    let users;

    let popul = [];
    if(populate !== null) popul = [...populate, 'author',  'author.name', 'author.wallet', 'author.id', 'author.avatar', 'author.avatar.url', 'author.banner', 'author.banner.url', 'author.nfts', 'preview_image', 'bids.author', 'bids.author.avatar', 'role', 'role.type'];
    else popul = ['author',  'author.name', 'author.wallet', 'author.id', 'author.avatar', 'author.avatar.url', 'author.banner', 'author.banner.url', 'author.nfts', 'preview_image', 'bids.author', 'bids.author.avatar', 'role', 'role.type'];

    if (_.has(ctx.query, '_q')) {
      // use core strapi query to search for users
      users = await strapi.query('user', 'users-permissions').search(ctx.query, {popul});
    } else {
  ...
  async findOne(ctx) {
    ...
    let populate = ['author',  'author.name', 'author.wallet', 'author.id', 'author.avatar', 'author.avatar.url', 'author.banner', 'author.banner.url', 'author.nfts', 'preview_image', 'bids.author', 'bids.author.avatar', 'role', 'role.type'];
